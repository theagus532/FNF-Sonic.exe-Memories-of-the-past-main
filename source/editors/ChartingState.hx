...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['HUD opaticity', "Ok so it basically tweens the HUD visibility \n\n Value 1: The visibility of the HUD \n Value 2: The time to tween it"],
		['Basic Speed Change', 'Changes the speed of all notes past this point\nValue 1: Multiplier'],
		['Song Speed Change', 'Changes the speed of all notes\nMay cause notes to shift\nWorks like normal Psych speed changes\nValue 1: Multiplier\nValue 2: Tween speed'],
		//['Speed Change', 'Changes the speed of all notes, but better\nValue 1: Multiplier.\n\nNOTE: If you use these, you cannot use Song Speed Changes'],
		...
		['YCR note', "crybit is holding me captive"],
		['Majin spin'],
		['Majin notestrum', "the funny hahaah"],
		['Majin count', "funny number countdown"],
		...
		['TDnoteshitdie', "eneterewyhusjdfgjesrdhjaerthgjkeraghreashgikuh"],
		['TDnoteshitlive', "i have risen from the grave"],
		['Genesis', "it does what nintendont lmaoiooaiojwuifghauyfgah"],
		['Character Fly', " "],
		['char disappear', " "],
		['char appear', " "],
		['Pnotefade','Fade'],
		['Pnotein', 'In'],
		['spingbing', "sponglebobbles"],
		['switch to xenophanes', "Goes to xenophanes stage"],
		['switch to normal', "back to funny normal vers of stage"],
		['strum swap1', "COCK"],
		['strum swap2', "BALLS"],
		['funnystatic', "@Srife5#1524 has 0 sex!!!!!!!!!!!!!!"],
		['startstatic', "comedy"],
		['tailsjump', "i have been coding triple trouble for 7 FUCKING HOURS\nthis is a new form of torture i swear"],
		['knuxjump', "im doing the jumpscares then going to bed FUCK YOU its 3am and ive been doing this since 4pm"],
		['eggjump', "eggballs ahgahhgahhaAGHAGHAHGAGHAHWAHGHGASDGHJWERSAGSHEJDJGHAGFGHADSHFGAGHDSFGAHFDSGHDFSHGSFDGHSFGHDSFHGAFGDGDFHASAGHFDHFSDHGFAFGHSDHFSD"],
		...
		["Chroma Video","Video that chromakeys out green\nValue 1: MP4 name\nThese won't show with flashing lights off because\nthis is mostly used for MLG shit!!"],
	...
	**/
	...
	override function create()
	{
		...
		if(_song.isRing)
			UI_box.x = FlxG.width / 2 + (GRID_SIZE + 120) / 2;
		else
	...
	function addSongUI():Void
	{
		...
		var check_ring = new FlxUICheckBox(10, 45, null, null, "5k", 100);
		...
		var cumFuckButton:FlxButton = new FlxButton(noteSplashesInputText.x + 5, noteSplashesInputText.y + 40, 'Hey 2 Static', function() {
			for (sec in 0..._song.notes.length) {
				var idx:Int = 0;
				for(note in _song.notes[sec].sectionNotes){
					if(note != null && note[1] > -1 && note[3] == 2)
						_song.notes[sec].sectionNotes[idx][3] = noteTypeList.indexOf("Static Note");
		...
		tab_group_song.add(cumFuckButton);
	...
	function addSectionUI():Void
	{
		...
		{
			var count:Int = 4;
			if(_song.isRing)count =5;
			for (i in 0..._song.notes[curSection].sectionNotes.length)
			{
				var note:Array<Dynamic> = _song.notes[curSection].sectionNotes[i];
				if(note[1]>-1){
					note[1] = (note[1] + count) % (count*2);
	...
	function reloadGridLayer() {
		...
		gridBG = FlxGridOverlay.create(GRID_SIZE , GRID_SIZE, GRID_SIZE * lol, Std.int(GRID_SIZE * 32 * zoomList[curZoom]));
		...
		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * poo)).makeGraphic(2, Std.int(gridBG.height), FlxColor.BLACK);
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSection].sectionNotes)
		{
			...
			if(note.noteData < 0) {
				...
				if(_song.isRing )
					dthing = 4;
				if(i[1] > dthing) note.mustPress = !note.mustPress;
			}
		}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		if(_song.isRing)
			...
			var note:Note = new Note(daStrumTime, daNoteInfo % lol, null, null, true);
	...
	function deleteNote(note:Note):Void
	{
		...
		if(_song.isRing)
			fucking = 5;
		if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSection].mustHitSection) noteDataToCheck += fucking;
